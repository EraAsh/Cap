	...
	const exportImage = async (destination: "file" | "clipboard" | "share") => {
		...
		try {
			if (destination === "share") {
				...
				if (!projectPath) throw new Error("截图仍在加载中");

				setExportStatus("encoding");
				toastId = toast.loading("准备上传中");
				...
				toast.loading("渲染截图中", { id: toastId });
			} else {
				setExportStatus("rendering");
			}
			...
			if (destination === "share" && toastId) {
				toast.loading("准备上传中", { id: toastId });
			}
			...
			if (destination === "file") {
				...
				if (toastId) toast.loading("上传截图中", { id: toastId });
				if (!shareContext) throw new Error("截图仍在加载中");
